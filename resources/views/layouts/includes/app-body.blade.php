<div>
       ...
       <nav class="bg-gradient-to-r from-red-600 via-orange-600 to-yellow-400 p-2 my-2" x-data="{ open: false }">
           <div class="container mx-auto flex justify-between items-center">
               <a href="{{ route('dashboard') }}" class="text-xl font-bold {{ request()->routeIs('dashboard') ? ' p-2 rounded' : '' }}">{{ config('app.name', 'Sharkni') }}</a>
               <div class="flex items-center space-x-4">
                ...
                <a href="{{ route('home') }}" class="block md:inline-block py-2 md:py-0 hover:underline text-center md:text-left {{ request()->routeIs('home') ? 'bg-yellow-500 p-2 rounded underline' : '' }}">{{ app()->getLocale() == 'ha' ? 'Gida' : 'Home' }}</a>
